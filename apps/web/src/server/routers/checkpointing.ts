...
function getCheckpointManager(db: Database) {
  return (_checkpointMgr ??= new CheckpointManager(db))
}
function getTimeTravelEngine(db: Database) {
  return (_timeTravelEngine ??= new TimeTravelEngine(db))
}
