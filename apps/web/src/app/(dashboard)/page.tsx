...
export default function CommandCenter() {
  ...
  const briefingQuery = trpc.intelligence.dailyBriefing.useQuery(undefined, {
  ...
  if (org && (org.stats.totalDepartments ?? 0) === 0) {
    router.push('/onboarding')
    return <LoadingState message="Setting up your corporation..." />
  }
  ...
  return (
    <div className="p-6 text-slate-50">
      ...
      {/* Welcome banner for recently onboarded users (1-2 departments) */}
      {org && org.stats.totalDepartments > 0 && org.stats.totalDepartments <= 2 && (
        <SectionCard variant="highlighted" className="mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            <div className="flex-1">
              <div className="text-sm font-medium text-slate-200">
                Welcome to {org.corporation.name}!
              </div>
              <div className="text-xs text-slate-400 mt-0.5">
                Your corporation is set up. Here&apos;s what to do next:
              </div>
            </div>
            <div className="flex gap-2">
              <Link href="/chat" className="cyber-btn-primary text-[10px] px-3 py-1 no-underline">
                Chat with Agents
              </Link>
              <Link
                href="/board"
                className="cyber-btn-secondary text-[10px] px-3 py-1 no-underline"
              >
                Create a Ticket
              </Link>
              <Link
      ...
      {/* Daily Briefing — generated by the intelligence system */}
      {briefingQuery.data && (
        <SectionCard title="Today's Briefing" variant="intelligence" className="mb-6">
          <div className="text-xs text-slate-400 leading-relaxed whitespace-pre-line">
            {(briefingQuery.data as string)
              .replace(/^# .+\n/m, '') // remove top-level heading (redundant with card title)
              .replace(/## /g, '') // simplify section headers
              .replace(/\*\*/g, '') // remove bold markdown (render as plain text)
              .trim()
              .slice(0, 800)}
          </div>
        </SectionCard>
