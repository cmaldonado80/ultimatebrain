...
const MOCK_RECORDING: QARecording = {
  ...
  annotations: [
    ...
    {
      timestampMs: 33000,
      label: 'Success toast shown',
      type: 'assertion',
      verdict: 'fail',
      screenshotIndex: 66,
      details: 'Toast not found within 3s',
    },
    {
      timestampMs: 40000,
...
const MOCK_REVIEW: ReviewResult = {
  ...
  checkpointResults: [
    {
      checkpoint: { name: 'Dashboard loaded', description: 'Dashboard page visible after login' },
      verdict: 'pass',
      confidence: 0.95,
      explanation: 'Dashboard elements detected',
      frameIndex: 24,
    },
    {
      checkpoint: { name: 'Profile form', description: 'Settings profile form is visible' },
      verdict: 'pass',
      confidence: 0.92,
      explanation: 'Form fields detected',
      frameIndex: 44,
    },
    {
      checkpoint: { name: 'Success toast', description: 'Success notification appears after save' },
      verdict: 'fail',
      confidence: 0.87,
      explanation: 'Toast element not found in frame',
      frameIndex: 66,
    },
    {
      checkpoint: { name: 'Dashboard redirect', description: 'Redirected back to dashboard' },
      verdict: 'pass',
      confidence: 0.91,
      explanation: 'Dashboard URL and content confirmed',
      frameIndex: 80,
    },
  ],
  failureFrames: [
    {
      frameIndex: 66,
      imageUrl: '/qa-recordings/rec-001/frame-66.png',
      offsetMs: 33000,
      reason: 'Success toast not visible after save action',
    },
  ],
  suggestedFixes: [
    {
      description: 'Add wait for toast notification after save',
      category: 'timing',
      priority: 'high',
      suggestion: 'await page.waitForSelector(".toast-success", { timeout: 5000 })',
    },
    {
      description: 'Verify toast component is rendering in settings page',
      category: 'ui',
      priority: 'medium',
    },
  ],
...
function FixRow({ fix }: { fix: SuggestedFix }) {
  const prioColor =
    fix.priority === 'high' ? '#ef4444' : fix.priority === 'medium' ? '#f97316' : '#6b7280'
  return (
...
export default function QAPlayer({
  ...
  const isDemo = recording === MOCK_RECORDING
  ...
  const currentFrameIdx = Math.min(Math.floor(currentMs / 500), recording.frames.length - 1)
  ...
  return (
    <div style={styles.container}>
      {isDemo && (
        ...
        >
          Demo Recording — connect to visual QA service for real data
        </div>
      )}
      {/* Header */}
        ...
        <div style={styles.playerSection}>
          <div style={styles.viewport}>
            <div style={styles.framePlaceholder}>
              <div style={styles.frameLabel}>
                Frame {currentFrameIdx + 1} / {recording.frames.length}
              </div>
          ...
          <div style={styles.controls}>
            ...
            <span style={styles.timeDisplay}>
              {formatMs(currentMs)} / {formatMs(recording.durationMs)}
            </span>
          </div>
          ...
          <div style={styles.timeline} onClick={handleTimelineClick}>
            <div
              style={{
                ...
                width: `${(currentMs / recording.durationMs) * 100}%`,
              }}
            />
          ...
          {activeAnnotation && (
            <div style={styles.annotationDetail}>
              ...
              {activeAnnotation.details && (
                <span style={styles.annDetails}>{activeAnnotation.details}</span>
              )}
            </div>
          )}
        </div>
