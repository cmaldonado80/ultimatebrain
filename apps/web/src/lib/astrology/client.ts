...
export function fetchNatalSummary(input: NatalSummaryInput): Promise<NatalSummaryResponse> {
  return post('/api/astrology/chart', input)
}
...
export function fetchReport(
  ...
  return post('/api/astrology/report', { ...birthData, ...options })
}
...
export function fetchTransits(
  ...
  return post('/api/astrology/transits', { ...birthData, ...range })
}
...
export function fetchTimeline(
  ...
  return post('/api/astrology/timeline', { ...birthData, period })
}
...
export function fetchSynastry(
  ...
  return post('/api/astrology/synastry', { personA, personB, narrative })
}
...
export async function saveChart(input: {
  ...
  return post('/api/astrology/charts', input)
}
...
export async function getChart(id: string): Promise<SavedChart> {
  const res = await fetch(`/api/astrology/charts/${id}`)
  if (!res.ok) throw new AstrologyBrainError('Chart not found', 404)
  return res.json()
}

export async function deleteChart(id: string): Promise<void> {
  await fetch(`/api/astrology/charts/${id}`, { method: 'DELETE' })
}
...
export async function saveReport(input: {
  ...
  return post('/api/astrology/reports', input)
}

export async function listReports(chartId?: string): Promise<SavedReport[]> {
  const url = chartId ? `/api/astrology/reports?chartId=${chartId}` : '/api/astrology/reports'
  const res = await fetch(url)
  if (!res.ok) return []
  return res.json()
}
...
export async function saveRelationship(input: {
  ...
  return post('/api/astrology/relationships', input)
}

export async function listRelationships(): Promise<SavedRelationship[]> {
  const res = await fetch('/api/astrology/relationships')
  if (!res.ok) return []
  return res.json()
}

export async function getReport(id: string): Promise<SavedReport> {
  const res = await fetch(`/api/astrology/reports/${id}`)
...
export async function getRelationship(id: string): Promise<SavedRelationship> {
  const res = await fetch(`/api/astrology/relationships/${id}`)
...
export async function createShareLink(
  ...
  return post('/api/astrology/share', { resourceType, resourceId })
}

export async function getSharedResource(token: string): Promise<Record<string, unknown>> {
  const res = await fetch(`/api/astrology/share/${token}`)
...
export async function getLastSeen(chartId: string): Promise<{ lastSeenAt: string } | null> {
  const res = await fetch(`/api/astrology/engagement?chartId=${chartId}`)
  if (!res.ok) return null
  return res.json()
}

export async function updateLastSeen(chartId: string): Promise<void> {
  await fetch('/api/astrology/engagement', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
